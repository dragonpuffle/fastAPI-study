from fastapi import FastAPI, BackgroundTasks, Cookie, Response, Request, Form
...
sample_products = [Product(**sample_product_1), Product(**sample_product_2), Product(**sample_product_3),
                   Product(**sample_product_4), Product(**sample_product_5)]
...
db_users = {"user123": {"password123"}


@app.get('/login', response_model=CommonMsg)
async def login(username: Form(...), password: Form(...), response: Response):
    if username in db_users and db_users[username] == password:
        session_token = random.randint(0, 1000000)
        response.set_cookie(key='session_token', value=f'{session_token}', secure=True, httponly=True)
        return {'msg': 'successfuly authorized'}
    return {'msg': 'Unauthorized'}

# РАЗОБРАТЬСЯ С КУКИ И ЖЕСТКИМ ПРИМЕРОМ СО СТЕПИКА
@app.get('/user')
